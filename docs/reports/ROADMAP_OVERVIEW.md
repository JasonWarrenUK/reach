...
**M3: Exploration & Close** holds the year-continuity question and the phase's closing act. The research spike here is allowed to conclude that full continuity isn't defensible and that a stepped control snapped to real calendar moments (the opening of the sailing season, the harvest, the closure) is the honest choice; that's a valid, planned outcome, not a failure of the spike. The write-up sits in this milestone too, combining the historical argument with the build story in one document. It's a reflection on the phase rather than a gate within it, but it does depend on the M2 and M3 implementation tasks finishing first: the build story can't be written until the build is done.
