...
- [ ] **2MD.6**: Design spike: comparison surface for two full models (overlay, delta readout or difference-as-projection) _(blocked: depends on 2MD.1, 2MD.3, 2MD.5)_
...
- [ ] **3EX.1**: Research spike: feasibility of translating the seasonal model into a continuous year control
  - Note: `SEASONS[season].forbid` is a discrete closed-mode list and `returnFactorAt` has no time term; true continuity needs either dated closures and a seasonal wind curve, or an honest admission that a stepped control snapped to real Roman calendar moments (sailing season opening, harvest, mare clausum) is the defensible option. A recommendation against full continuity, in favour of the stepped version, is an acceptable spike outcome. Output: a written recommendation for approval.
- [ ] **3EX.2**: Implement the year/time control per the spike's recommendation _(blocked: depends on 3EX.1)_
- [ ] **3EX.3**: Write the concept and process write-up (the historical argument and the build story, one document)
  - Note: close to final in the phase, not gating anything; covers both what the toy is arguing about antiquity and what building it involved, combining devlog and concept framing in one document.
