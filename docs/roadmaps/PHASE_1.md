...
- [ ] **3EX.2**: Implement the year/time control per the spike's recommendation
  - Note: per [`docs/spikes/3ex1-year-control.md`](../spikes/3ex1-year-control.md) (approved 2026-09-17): a `YearBand` control with four segments bounded by Vegetius 4.39 (10 March, 27 May, 14 September, 11 November), widths proportional to day counts, band starting at 10 March. `SeasonKey` stays three-valued; segments map onto it. Hash param `s` takes `spring`, `sailing`, `autumn`, `winter`, with legacy `shoulder` read as `spring`. No time term on `returnFactorAt`, no harvest marks, no day-level scrubbing.
